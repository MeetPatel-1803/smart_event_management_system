...
export class AutoMigration1789622753319 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."user_events_status_enum" AS ENUM('EXPIRED', 'CANCELLED', 'WAITLISTED', 'REGISTERED', 'PAYMENT_PENDING')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."user_events_status_enum_old" AS ENUM('REGISTERED', 'CANCELLED', 'PENDING', 'PAYMENT_PENDING')`,
    );
    await queryRunner.query(
